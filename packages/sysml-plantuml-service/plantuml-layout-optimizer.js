...
const { layoutDiagram } = require('./diagram-layout');
...
async function optimizePlantUmlLayout({ content, viewName = '', renderMode = '', renderCandidate, renderScene, deadlineAt }) {
  ...
  const defaultCandidate = await renderAndScore({ content, viewName, renderMode, strategy: defaultStrategy, diagramKind, renderCandidate, renderScene, deadlineAt });
  candidates.push(defaultCandidate.summary);

  if (defaultCandidate.result.renderer === 'local-layout' || defaultCandidate.result.renderer === 'engineering-native') {
    return { ...defaultCandidate.result, layoutOptimization: {
      mode: 'engineering', selected: defaultCandidate.result.renderer === 'local-layout' ? 'ELK_ORTHOGONAL' : 'PLANTUML_NATIVE', attempted: 1,
      switched: false, candidates, reason: '官方语义投影采用图种布局与统一皮肤，由 PlantUML 输出 SVG。'
    } };
  }
...
async function renderAndScore({ content, viewName, renderMode, strategy, diagramKind, renderCandidate, renderScene, deadlineAt }) {
  ...
  result = checkRenderResult(result);
  const effectiveDiagramKind = diagramKindFromResolvedMode(result.resolvedRenderMode) || diagramKind;
  if (result.kind === 'DIAGRAM_DOCUMENT' && result.diagramDocument?.status === 'ready') {
    try {
      const layout = await layoutDiagram(result.diagramDocument, {
        timeoutMs: deadlineAt ? deadlineAt - Date.now() : 10000, geometryOnly: true
      });
      if (typeof renderScene !== 'function') throw Object.assign(new Error('PlantUML 绘图后端不可用。'), { code: 'DRAWING_UNAVAILABLE' });
      const drawing = checkRenderResult(await renderScene(layout.scene, deadlineAt ? deadlineAt - Date.now() : undefined));
      if (!drawing.ok || !drawing.svg) throw Object.assign(new Error('PlantUML 未返回有效绘图。'), { code: 'DRAWING_FAILED' });
      result = { ...result, ok: true, kind: 'SVG', svg: drawing.svg, renderer: 'local-layout', drawingBackend: 'plantuml-ugraphic',
        source: 'official-sysml-v2-model-local-layout', backend: 'official-model-elk',
        layoutVersion: layout.version, geometryHash: layout.geometryHash,
        layoutScene: layout.scene, geometryQuality: layout.quality,
        interconnectionLayout: effectiveDiagramKind === 'interconnection' ? {
          applied: true, reason: 'official-model-elk', metrics: {
            partOverlapCount: 0, nonOrthogonalConnectorCount: 0, unresolvedConnectorCount: 0,
            connectorCount: layout.scene.edges.length
          }
        } : undefined };
    } catch (error) {
      result = { ...result, ok: false, kind: 'ERROR', renderer: 'local-layout', svg: '',
        diagnostics: [{ severity: 'error', code: error.code || 'LAYOUT_ENGINE_ERROR', message: error.message }] };
    }
  }
  if (result.ok && result.svg && result.renderer !== 'local-layout' && effectiveDiagramKind === 'interconnection') {
...
function checkRenderResult(result) {
  if (!result || !result.ok || result.kind === 'DIAGRAM_DOCUMENT') return result;
  const svg = String(result.svg || '');
  const errorSvg = /data-diagram-type=["']ERROR["']/i.test(svg)
    || /<text\b[^>]*>\s*(?:Syntax Error\??|An error has occurred|Error line \d+)\s*<\/text>/i.test(svg);
  const empty = result.kind === 'EMPTY' || !svg.trim() || /<svg\b[^>]*viewBox=["']0 0 10 10["']/i.test(svg);
  if (!errorSvg && !empty) return result;
  return { ...result, ok: false, kind: errorSvg ? 'ERROR' : 'EMPTY', svg: '',
    diagnostics: [...(result.diagnostics || []), { severity: 'error',
      code: errorSvg ? 'PLANTUML_ERROR_SVG' : 'EMPTY_VIEW',
      message: errorSvg ? 'PlantUML 返回了错误图，未生成模型视图。' : '所选视图没有可显示的内容。' }] };
}
